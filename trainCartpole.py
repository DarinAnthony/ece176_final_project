# needs to be fixed by changing up the replayBuffer to work with vector based inputs, 
# and also remove the conv layers since states are not images
...
def train_cartpole_dqn(num_frames=10000000,
...
if __name__ == "__main__":
    train_cartpole_dqn(num_frames=2000,
        memory_size=50000,          # Smaller replay buffer
        batch_size=32,
        gamma=0.99,
        eps_start=1.0,
        eps_end=0.02,
        eps_decay=10000,             # Very fast epsilon decay
        target_update=1000,          # Update target network frequently
