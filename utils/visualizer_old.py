...
class DQNVisualizer:
    ...
    def __init__(self, 
        ...
        # Create env-specific directory structure
        env_base_name = env_name.replace('/', '_')
        self.env_dir = os.path.join(output_dir, env_base_name)
        self.videos_dir = os.path.join(self.env_dir, "videos")
        self.metrics_dir = os.path.join(self.env_dir, "metrics")
    ...
    def record_episode(self, filename=None, max_steps=10000, render=True):
        ...
        if filename is None:
            ...
            filename = f"{self.env_name.replace('/', '_')}_{timestamp}"
        
        # Create environment
        env = gym.make(self.env_name, render_mode='rgb_array')
        

        observation = self.agent.reset_environment()
